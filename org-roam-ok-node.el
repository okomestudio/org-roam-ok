...
;;; Caching complete read items

(defvar oron-display--cache (make-hash-table :test #'equal)
  "In-memory cache for org-roam-node display.")

(defun oron-display--cache-get (node total-width)
  "Get the cached item for NODE and TOTAL-WIDTH from in-memory cache."
  (if-let* ((id (org-roam-node-id node))
            (title (org-roam-node-title node))
            (entries (gethash id oron-display--cache))
            (value (cdr (assoc `(,title ,total-width) entries))))
      value))

(defun oron-display--cache-save (value node total-width)
  "Save VALUE to cache for NODE and TOTAL-WIDTH."
  (let* ((id (org-roam-node-id node))
         (title (org-roam-node-title node))
         (entries (gethash id oron-display--cache)))
    (push (cons `(,title ,total-width) value) entries)
    (puthash id entries oron-display--cache)))

(defun oron-display--cache-remove (node)
  "Remove NODE item from the in-memory cache."
  (remhash (org-roam-node-id node) oron-display--cache))

(defun oron-display--cache-clear ()
  "Clear the in-memory cache."
  (interactive)
  (clrhash oron-display--cache))
...
(defun oron-display-cache-rendered (fun node &rest rest)
  "Cache rendered display value for NODE.
Use this as an around advice for `org-roam-node-display-template'
FUN."
  (if-let* ((total-width (frame-width))
            (cached (oron-display--cache-get node total-width)))
      cached
    (let ((rendered (apply fun `(,node ,@rest))))
      (oron-display--cache-save rendered node total-width)
      rendered)))

(cl-defun oron-display-template (node &optional total-width)
  "Render NODE using display template.
When not given, TOTAL-WIDTH defaults to the current `frame-width'.

Use this as the default display template function."
